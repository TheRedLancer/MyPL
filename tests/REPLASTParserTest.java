...
public class REPLASTParserTest {
  //------------------------------------------------------------
  // HELPER FUNCTIONS
  //------------------------------------------------------------
  
  private static REPLASTParser buildParser(String s) throws Exception {
  ...
  public void emptyParse() throws Exception {
    REPLASTParser parser = buildParser("");
    REPLProgram p = parser.parse();
    assertEquals(0, p.tdecls.size());
    assertEquals(0, p.fdecls.size());
    assertEquals(0, p.stmts.size());
  }
  ...
  public void oneTypeDeclInProgram() throws Exception {
    String s = buildString
      ("type Node {",
       "}");
    REPLASTParser parser = buildParser(s);
    REPLProgram p = parser.parse();
  ...
  //-----------------------------------------
  @Test
  public void outsideInt() throws Exception {
    String s = buildString
      ("10");
    REPLASTParser parser = buildParser(s);
    REPLProgram p = parser.parse();
    assertEquals(0, p.fdecls.size());
    assertEquals(2, p.stmts.size());
    assertEquals(AssignStmt.class, p.stmts.get(0).getClass());
    assertEquals(CallExpr.class, p.stmts.get(1).getClass());
  }

  @Test
  public void outsideAdd() throws Exception {
    String s = buildString
      ("10 + 12");
    REPLASTParser parser = buildParser(s);
    REPLProgram p = parser.parse();
  ...
  @Test
  public void outsideIf() throws Exception {
    String s = buildString("",
        "if x < 2 {",
            "print(12)",
        "} else {",
    ...
    REPLProgram p = parser.parse();
    assertEquals(0, p.fdecls.size());
    assertEquals(1, p.stmts.size());
    assertEquals(CondStmt.class, p.stmts.get(0).getClass());
    assertEquals(CallExpr.class, ((CondStmt)p.stmts.get(0)).ifPart.stmts.get(0).getClass());
    assertEquals(AssignStmt.class, ((CondStmt)p.stmts.get(0)).elseStmts.get(0).getClass());
  }

  @Test
  public void attemptUseIt() throws Exception {
    String s = buildString("",
        "print(it)");
    REPLASTParser parser = buildParser(s);
    try {
  ...
  @Test
  public void exprInFunction() throws Exception {
    String s = buildString("",
        "fun void main() {",
          "2+2",
        "}"
